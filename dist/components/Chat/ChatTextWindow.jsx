...
import { chatWindowState, chatWelcomeMessageState, chatUserIsTypingState, } from "../../store/chat";
import { usernameState, userLeftChattrState, listUsersState, } from "../../store/users";
...
const ChatTextWindow = () => {
    ...
    return (<Wrapper>
      <ScrollArea style={{
        ...
        borderRadius: "5px",
    }} verticalScrollbarStyle={{ background: "#000" }} verticalContainerStyle={{ background: "#eee" }} horizontal={false}>
        <Container>
          <AnimatePresence>
            {msgs.length > 0 &&
        msgs.map(({ msg, user }, i) => (<MsgWrapper key={i} initial={{ y: 5 }} animate={{ y: 0 }} exit={{ opacity: 0, transition: { duration: 0 } }} transition={{ type: "spring", damping: 80 }}>
                  <Username me={username === user}>{user}</Username>
                  <span>{msg}</span>
                </MsgWrapper>))}
          </AnimatePresence>
          {msgs.length === 0 && (<NoMessages>
              <FaKiwiBird size={32} style={{ marginBottom: 15 }} color="var(--textColor)"/>
              <NoMessagesText>{welcomeMsg}</NoMessagesText>
            </NoMessages>)}

          {userIsTyping?.status && username !== userIsTyping?.username && (<UserIsTypingWrapper initial={{ y: 5 }} animate={{ y: 0 }} transition={{ type: "spring", damping: 80 }}>
              <UserIsTypingText>
                {userIsTyping?.username} is typing...
              </UserIsTypingText>
            </UserIsTypingWrapper>)}

          {userLeftChattr && (<UserDisconnectedWrapper initial={{ y: 5 }} animate={{ y: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", damping: 80 }}>
...
const Username = styled.span `
  color: ${(props) => props.me ? "var(--tertiaryColor)" : "var(--secondaryColor)"};
  font-weight: 600;
`;
