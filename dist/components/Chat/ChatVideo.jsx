...
import { showSelfWebcamState, receivingCallState, callAcceptedState, pressedCallState, } from "../../store/video";
import { listUsersState } from "../../store/users";
const ChatVideo = ({ acceptCall, selfVideoRef, friendVideoRef, socket, }) => {
    ...
    return (<Wrapper ref={contraintsRef}>
      <>
        ...
        !callAccepted &&
        !pressedCall &&
        listUsers?.length >= 2 && (<ChatScreenNoVideo setShowCatSlider={setShowCatSlider}/>)}

            {receivingCall && !callAccepted && (<ChatScreenIncomingCall acceptCall={acceptCall} socket={socket}/>)}
          </>) : (<AnimatePresence>
            <Slider setShowCatSlider={setShowCatSlider}/>
          </AnimatePresence>)}
      </>
      <>
        <SelfVideo muted initial={{ scaleX: -1 }} exit={{ scaleX: 0 }} drag dragMomentum={false} 
    // @ts-ignore
    dragConstraints={contraintsRef} ref={selfVideoRef} playsInline autoPlay style={{ opacity: showWebcam ? 1 : 0 }}/>
